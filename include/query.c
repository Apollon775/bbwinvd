...
{
    char stm[BUFFER];
    sprintf(stm, "UPDATE fiae2019 SET OS = \'%s\' WHERE Hostname = \'%s\'; IF @@ROWCOUNT = 0  THEN INSERT INTO fiae2019(Hostname, OS) VALUES(\'%s\', \'%s\'); ",
    ...
    {    
        sprintf(stm, "UPDATE interfaces SET IPv4 = \'%s\', Hostname = \'%s\', Eingegeben am = NOW() WHERE MAC = \'%s\' ; IF @@ROWCOUNT = 0 THEN INSERT INTO interfaces(MAC, IPv4, Hostname) VALUES(\'%s\', \'%s\', \'%s\') ; ",
